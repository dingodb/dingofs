...
void CompletionChannel::Reset() noexcept {
  if (channel_ != nullptr) {
    ...
    if (const int rc = ibv_destroy_comp_channel(channel_); rc != 0) {
...
StatusOr<CompletionChannel> CompletionChannel::Create(Device& device) {
  ...
  if (flags == -1 || fcntl(channel->fd, F_SETFL, flags | O_NONBLOCK) == -1) {
    Status status = ToStatus(errno, "set the completion channel non-blocking");
