...
inline const std::vector<FlagSection> kBenchSections = {
    {"BENCH OPTIONS",
     {"op", "threads", "inflight", "fsid", "blksize", "blocks", "offset",
      "length", "stage", "retrieve_storage", "runtime", "time_based", "conf"}},
    {"CLIENT OPTIONS",
     {"cache_store", "enable_stage", "enable_cache", "fill_group_cache",
      "queue_depth", "shards", "cpuset", "pin_cpu", "poll_mode", "idle_poll_us",
      "task_quota_us", "iodepth", "buffer_pool_mb", "offload_threads",
      "offload_queue_capacity", "offload_cpu_min_bytes", "offload_cpu_spin_us",
    ...
    {"REMOTE CACHE OPTIONS",
     {"cache_group", "periodic_sync_members_ms", "cache_rpc_timeout_ms",
      "cache_rpc_connect_timeout_ms", "cache_rpc_max_retry_times",
...
inline const FlagParser::Usage kBenchUsage = {
    ...
    .examples =
        "  $ cb --mds_addrs=10.0.0.1:6900 --op=put --threads=4 --inflight=16\n"
        "  $ cb --mds_addrs=10.0.0.1:6900 --op=get --time_based --runtime=60\n",
    .sections = kBenchSections,
    .essential = {"op", "threads", "inflight", "fsid", "blksize", "blocks",
                  "stage", "retrieve_storage", "runtime", "time_based", "conf",
                  "mds_addrs", "cache_store", "cache_group", "use_rdma",
                  "cache_dir", "log_dir"},
    .required = {},
    .uuid_flag = "",
};
