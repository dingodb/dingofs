...
void CacheStatus::Set(SetFunc set_func) {
  // WriteLockGuard lock(CacheStatus::rwlock_);
  // set_func(root_);
}

std::string CacheStatus::Dump() {
  return "";
  // ReadLockGuard lock(CacheStatus::rwlock_);
  // return ToJSON(CacheStatus::root_).dump();
}

nlohmann::json CacheStatus::ToJSON(Health health) {
  nlohmann::json msg;
  msg["state"] = health.state;
  msg["last_modified"] = health.last_modified;
  msg["changelog"] = health.changelog;
  return msg;
}
