...
class CacheStatus {
 public:
  ...
  struct Health {
    std::string state;  // normal, unstable, down
  ...
  using SetFunc = std::function<void(Root& root)>;
  static void Set(SetFunc set_func);
  static std::string Dump();

 private:
  static nlohmann::json ToJSON(Disk disk);
  static nlohmann::json ToJSON(Health health);
