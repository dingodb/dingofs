...
InfinibandServiceImpl::~InfinibandServiceImpl() {
...
void InfinibandServiceImpl::Sync(google::protobuf::RpcController* controller,
  ...
  // remote connection managment meta
  ConnManagmentMeta remote_cm_meta;
  auto status = Helper::ParseFromPb(request->cm_meta(), &remote_cm_meta);
  if (!status.ok()) {
  ...
  Helper::SerializeToPb(local_cm_meta, response->mutable_cm_meta());
