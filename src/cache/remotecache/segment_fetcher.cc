...
Segment* BlockMap::Block::GetOrCreateSegment(int index) {
  ...
  segment = new (std::nothrow) Segment(index);  // TODO(Wine93): handle new fail
...
SegmentFetcher::SegmentFetcher(Cache* cache, RemoteCacheNodeSPtr remote_node)
    : cache_(cache), remote_node_(remote_node) {}
...
SegmentFetcher::Task* SegmentFetcher::GetTaskEntry(const BlockKey& key,
                                                   size_t block_length,
                                                   Segment* segment) {
  if (segment->SetFetching()) {
    auto* task = new Task();
    task->block_key = key;
...
void SegmentFetcher::DoFetch(Task* task) {
  ...
  off_t offset = segment->GetIndex() * kSegmentSize;
  size_t length = std::min(task->block_length - offset, kSegmentSize);  // FIXME
  auto status = remote_node_->Range(nullptr, task->block_key, offset, length,
                                    buffer, RangeOption());
...
void SegmentFetcher::OnSuccess(Task* task, Status /*status*/,
                               IOBuffer* buffer) {
  VLOG(3) << "Success to fetch segment: key = " << task->block_key.Filename()
...
Status SegmentHandler::Handle(const BlockKey& key, off_t offset, size_t length,
  ...
  off_t soff_l = index * kSegmentSize;
  ...
  if (handle != nullptr) {
    ...
    auto* sbuffer = static_cast<IOBuffer*>(cache_->Value(handle));
...
Status CacheRetriever::Range(const BlockKey& key, off_t offset, size_t length,
  ...
  auto tindex = SegmentIndex(offset + block_length);
