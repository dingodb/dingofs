  ...
  void Execute(const std::string& method,
    ...
    if (method == "Range") {
      ...
      if (resp_body != nullptr && resp_body->Size() > 0) {
        ...
        region.set_addr(reinterpret_cast<uint64_t>(resp_body->Fetch1()));
        region.set_length(static_cast<uint32_t>(resp_body->Size()));
        region.set_rkey(static_cast<uint32_t>(resp_body->GetFirstDataMeta()));
      }
      ...
      if (req_body != nullptr && req_body->Size() > 0) {
        ...
        if (body->ConstIOBuf().backing_block_num() == 1) {
          ...
          region->set_addr(reinterpret_cast<uint64_t>(body->Fetch1()));
          region->set_length(static_cast<uint32_t>(body->Size()));
          region->set_rkey(static_cast<uint32_t>(body->GetFirstDataMeta()));
        } else {
          ...
          for (const auto& iov : body->Fetch()) {
            ...
            region->set_rkey(registry.RkeyFor(iov.iov_base, iov.iov_len));
          }
        }
      }
    }
