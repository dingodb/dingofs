...
Status BlockCacheImpl::Range(BlockHandle handle, off_t offset, size_t length,
  ...
  // Storage primitive: fills a caller-allocated `buffer` in place when given one
  // (client local tier), or allocates and hands it back when empty (the cache
  // server reads lazily into a send slab -- zero copy, no waste on a miss).
