...
void PutClosure::Run() {
  ...
  if (option_.async_cache_func) {
    option_.async_cache_func(key_, Block(buffer));
  }

  auto retry_cb = [this, buffer](int retcode) {
  ...
  block_accesser_->AsyncPut(key_.Filename(), buffer.Fetch1(), block_.size,
                            retry_cb);
}
...
// Copy data to continuous memory
IOBuffer PutClosure::CopyBuffer() {
...
void RangeClosure::Run() {
  ...
  *buffer_ = IOBuffer(iobuf);
  ...
  block_accesser_->AsyncRange(key_.Filename(), offset_, length_,
