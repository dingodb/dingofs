...
void AioQueueImpl::OnError(Aio* aio, Status status) {
  CHECK_NE(GetStep(aio), kExecuteIO) << absl::StrFormat(
      "Aio %s it not on expected phase: got(%s) != expect(%s)", aio->ToString(),
      GetStep(aio), kExecuteIO);
...
void AioQueueImpl::OnCompleted(Aio* aio) {
  CHECK_EQ(GetStep(aio), kExecuteIO) << absl::StrFormat(
      "Aio %s it not on expected phase: got(%s) != expect(%s)", aio->ToString(),
      GetStep(aio), kExecuteIO);
...
std::string AioQueueImpl::GetStep(Aio* aio) {
  return aio->ctx->GetTimer().GetStep();
}
