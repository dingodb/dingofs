...
void RegisterFlags(FlagSet* fs, Options* o) {
  ...
  fs->Size("bs", &o->bs, "block size = whole-file size, <= 4MiB");
  fs->U64("nrfiles", &o->nrfiles, "number of distinct block files (key space)");
  ...
  fs->Switch("prep", &o->prep, "pre-write all blocks before read tests");
...
std::string Validate(Options* o) {
  ...
  if (o->bs == 0 || o->bs > 4ULL * 1024 * 1024) {
    return "bs must be in (0, 4MiB]";
  }
