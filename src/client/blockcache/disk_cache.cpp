...
Status DiskCache::Init(UploadFunc uploader) {
  ...
  if (!aio_queue_->Init(option_.ioring_iodepth)) {
    return Status::Internal("init io ring failed.");
  }
...
Status DiskCache::Shutdown() {
  ...
  if (!aio_queue_->Shutdown()) {
    return Status::Internal("shutdown aio queue failed");
  }
...
Status DiskCache::NewBlockReader(const BlockKey& key,
  ...
  return fs_->Do([&](const std::shared_ptr<PosixFileSystem> posix) {
    ...
    if (option_.filesystem_type == "3fs") {
      reader = std::make_shared<RemoteBlockReader>(fd, option_.ioring_blksize, fs_, aio_queue_);
    } else {
