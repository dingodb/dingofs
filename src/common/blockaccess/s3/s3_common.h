...
struct AwsSdkConfig {
  ...
  // 0 means the sdk performs exactly one attempt per request: retry is owned
  // by the upper storage client so failures back off instead of stacking
  // sdk-internal resends under an application-level retry loop.
