...
void AwsCrtS3Client::AsyncDeleteObject(const std::string& bucket,
                                       const std::string& key,
  ...
  DeleteObjectResponseReceivedHandler handler =
      [bucket, key](
        ...
        LOG_IF(ERROR, !response.IsSuccess()) << fmt::format(
            "[s3_crt.{}] AsyncDeleteObject fail, key({}) error({} {}).", bucket,
            key, response.GetError().GetExceptionName(),
            response.GetError().GetMessage());
...
void AwsCrtS3Client::AsyncDeleteObjects(
  ...
  DeleteObjectsResponseReceivedHandler handler =
      [bucket](
        ...
        if (!response.IsSuccess()) {
          LOG(ERROR) << fmt::format(
              "[s3_crt.{}] AsyncDeleteObjects fail, error({} {}).", bucket,
              response.GetError().GetExceptionName(),
              response.GetError().GetMessage());
          ...
          for (const auto& err : response.GetResult().GetErrors()) {
            LOG(WARNING) << fmt::format(
                "[s3_crt.{}] delete fail, key({}) error({} {}).", bucket,
