...
#ifndef DINGOFS_SRC_COMMON_OPTIONS_H_
#define DINGOFS_SRC_COMMON_OPTIONS_H_
...
#define BIND_any(T, name, default_value, comment)                          \
 ...
 private:                                                                  \
  ...
  common::Item<T> name##Item_ = common::Item<T>(#name, &name##_, comment); \
  bool name##Insert_ = [this]() {                                          \
    BaseOption::items_[#name] =                                            \
        reinterpret_cast<common::IItem*>(&this->name##Item_);              \
    return true;                                                           \
  }()
...
#define BIND_FLAG_any(T, name, gflag_name)                    \
 public:                                                      \
  ...
  void set_##name(T value) { FLAGS_##gflag_name = value; }    \
                                                              \
 private:                                                     \
  common::Item<T> name##Item_ =                               \
      common::Item<T>(#name, &FLAGS_##gflag_name, "");        \
  bool name##Insert_ = [this]() {                             \
    BaseOption::items_[#name] =                               \
        reinterpret_cast<common::IItem*>(&this->name##Item_); \
    return true;                                              \
...
#define BIND_suboption(name, child_name, cls)                  \
 ...
 private:                                                      \
  cls name##_;                                                 \
  bool name##Insert_ = [this]() {                              \
    common::BaseOption::childs_[child_name] =                  \
        reinterpret_cast<common::BaseOption*>(&this->name##_); \
    return true;                                               \
  }()
...
#define REWRITE_BY(rewrite_func)                      \
 private:                                             \
  bool Rewrite_ = [this]() {                          \
    common::BaseOption::rewrite_func_ = rewrite_func; \
    return true;                                      \
  }()
...
#define VALIDATE_BY(validate_func)                      \
 private:                                               \
  bool Validate_ = [this]() {                           \
    common::BaseOption::validate_func_ = validate_func; \
    return true;                                        \
  }()
...
#endif  // DINGOFS_SRC_COMMON_OPTIONS_H_
