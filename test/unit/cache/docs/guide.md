...
2. 你需要单元测试能覆盖到所有的源码文件，尽可能确保测试的覆盖率（运行当前目录下的覆盖率统计脚本 run_test.sh，覆盖率至少要 90% 以上）
3. 测试需要尽可能考虑到各种 `case`，比如一个 `rpc` 调用成功或失败
4. 对于一些 IO 读写的实现，你需要添加一些单元测试来测试其性能（比如输出读写一个 4MB 文件的耗时）
5. 在编写完单测后，你需要完成编译，并运行所有的单元测试，确保所有单元测试都能顺利通过测试
6. 此外，在你编写测试用例后，我会直接在源码中以 `// REVIEW` 的特定方式提交一些 review 意见，你需要根据这些意见修改对应的实现，如：
...
2. 如果一个用例超过 60 秒没有结果，你需要检查一下是否是死锁（或死循环）还是测试用例本身需要等待这么久。如果是死锁（或死循环了），你需要手动停止运行，并修改对应的实现来修复死锁（或死循环）。如果测试用例本身需要等待这么久，你需要在测试用例中添加一些注释来说明为什么需要等待这么久
