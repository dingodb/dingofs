...
// Throws on every run. A worker must survive it, keep its pending count honest
// and go on to the next task: the exception must never cross the bthread
// execution queue boundary.
class ThrowTask : public TaskRunnable {
 public:
  std::string Type() override { return "THROW"; }
...
TEST(RelayWorkerSetTest, HashKeepsSubmissionOrder) {
  constexpr uint32_t kWorkerNum = 4;
  constexpr uint32_t kInoNum = 4;
  constexpr uint64_t kTasksPerIno = 2000;

  RelayWorkerSet worker_set("fast_order", kWorkerNum);
...
TEST(RelayWorkerSetTest, StopDrainsQueuedTasks) {
  constexpr int64_t kTaskNum = 200;

  // One downstream worker: the gate freezes the task that worker is running,
  // so every other task is still queued when Stop() is called.
  RelayWorkerSet worker_set("fast_drain", 1);
  ASSERT_TRUE(worker_set.Init());
  ...
  // Nothing can have completed while the gate is shut. The relay may already
  // have handed the tasks to the downstream set, so pending is not a useful
  // signal here -- the run count is.
  EXPECT_EQ(total.load(std::memory_order_relaxed), 0);
...
TEST(RelayWorkerSetTest, ExecuteHashPerformance) {
  ...
  RelayWorkerSet worker_set("fast_perf", kWorkerNum);
  ASSERT_TRUE(worker_set.Init());
  ...
  std::cout << "RelayWorkerSet ExecuteHash: workers=" << kWorkerNum
...
TEST(RelayWorkerSetTest, ExecqSubmitBaseline) {
  if (getenv("MANUAL_TEST") == nullptr) {
    GTEST_SKIP() << "Skip manual test case.";
  }
  ...
  WorkerSetUPtr worker_set =
      ExecqWorkerSet::NewUnique("execq_perf", kWorkerNum, 0, true);
  ASSERT_TRUE(worker_set->Init());
  ...
  for (int64_t p = 0; p < kProducerNum; ++p) {
    producers.emplace_back([&, p] {
      ...
      for (int64_t i = 0; i < kPerProducer; ++i) {
        const uint64_t id = static_cast<uint64_t>(p * kPerProducer + i);
        if (!worker_set->ExecuteHash(static_cast<int64_t>(id), tasks[p][i])) {
          rejected.fetch_add(1, std::memory_order_relaxed);
        }
      }
    });
  ...
  std::cout << "ExecqWorkerSet ExecuteHash baseline: workers=" << kWorkerNum
            << " producers=" << kProducerNum << " tasks=" << kTaskNum << "\n"
            << "  submit: " << submit_us << " us, "
            << (kTaskNum * 1000000.0 / submit_us) << " tasks/s, "
            << (submit_us * 1000.0 / kTaskNum) << " ns/task\n";
...
TEST(RelayWorkerSetTest, ThrowingTaskDoesNotKillTheWorker) {
  // Both tasks hash to the same worker, so the throwing one runs first.
  RelayWorkerSet worker_set("fast_throw", 2);
  ASSERT_TRUE(worker_set.Init());

  std::atomic<int64_t> total{0};
  std::atomic<int64_t> run_count{0};

  ASSERT_TRUE(worker_set.ExecuteHash(1, std::make_shared<ThrowTask>()));
  ASSERT_TRUE(worker_set.ExecuteHash(
      1, std::make_shared<CountTask>(&total, &run_count)));

  EXPECT_TRUE(WaitUntil(
      [&] { return total.load(std::memory_order_relaxed) == 1; }));
...
TEST(RelayWorkerSetTest, ExecuteAfterStopIsRejected) {
  RelayWorkerSet worker_set("fast_rejected", 2);
  ASSERT_TRUE(worker_set.Init());
