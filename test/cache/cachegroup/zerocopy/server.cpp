...
int OpenFile(const std::string& filepath) {
  ...
  auto rc = fs->Open(filepath, O_RDWR | O_DIRECT | O_CREAT, &fd);
  LOG_IF(FATAL, fd <= 0) << "Open file failed, filepath=" << filepath;
  return fd;
}
...
int main(int argc, char** argv) {
  ...
  */

  {
    k_timer.start();
    char* data =
        (char*)::mmap(nullptr, 4 * kMiB, PROT_READ, MAP_PRIVATE, fd, 0);
    ...
    //::butil::IOBuf iobuf;
    // auto* data2 = AllocBuffer(4 * kMiB);
    ////  iobuf.append_user_data(data2, 4 * kMiB, nullptr);
    // iobuf.append_user_data(data, 4 * kMiB, nullptr);
    //    k_timer.stop();
    //    LOG(INFO) << "append() " << k_timer.u_elapsed() * 1.0 / 1e6 << "
    //    seconds.";

    // k_timer.start();
    int out_fd = OpenFile(argv[2]);
    // k_timer.stop();
    // LOG(INFO) << "open() " << k_timer.u_elapsed() * 1.0 / 1e6 << " seconds.";

    AioContext w_aio(AioType::kWrite, out_fd, 0, 4 * kMiB, data, AioCallback);
    aio_queue->Submit(std::vector<AioContext*>{&w_aio});
