...
TEST_F(LocalFileSystemTest, Walk) {
  ...
  ASSERT_EQ(fs->WriteFile(PathJoin({root_dir_, "a"}), "x", 1), Errno::OK);
  ASSERT_EQ(fs->WriteFile(PathJoin({root_dir_, "b"}), "x", 1), Errno::OK);
  ASSERT_EQ(fs->WriteFile(PathJoin({root_dir_, "c"}), "x", 1), Errno::OK);
...
TEST_F(LocalFileSystemTest, WriteFile) {
  ...
  ASSERT_EQ(fs->WriteFile(path, "x", 1), Errno::OK);

  size_t count;
  std::shared_ptr<char> buffer;
  ASSERT_EQ(fs->ReadFile(path, buffer, &count), Errno::OK);
  ASSERT_EQ(count, 1);
  ASSERT_EQ(std::string(buffer.get(), count), "x");

  ASSERT_EQ(fs->WriteFile(path, "yy", 2), Errno::OK);
  ASSERT_EQ(fs->ReadFile(path, buffer, &count), Errno::OK);
  ASSERT_EQ(count, 2);
...
TEST_F(LocalFileSystemTest, RemoveFile) {
  ...
  ASSERT_EQ(fs->RemoveFile(path), Errno::OK);
...
TEST_F(LocalFileSystemTest, HardLink) {
  ...
  ASSERT_EQ(fs->WriteFile(src, "x", 1), Errno::OK);
  ASSERT_TRUE(fs->FileExists(src));
  ASSERT_FALSE(fs->FileExists(dest));

  ASSERT_EQ(fs->HardLink(src, dest), Errno::OK);
...
TEST_F(LocalFileSystemTest, FileExists) {
  ...
  ASSERT_EQ(fs->MkDirs(dir), Errno::OK);
  ASSERT_FALSE(fs->FileExists(dir));
