...
TEST_F(DiskCacheManagerTest, CleanupFull) {
  ...
  ASSERT_EQ(disk_cache->Stage(key_100, block, ctx), Errno::OK);
  ASSERT_EQ(disk_cache->Stage(key_200, block, ctx), Errno::OK);
  ...
  ASSERT_EQ(disk_cache->Stage(key_300, block, ctx), Errno::OK);
  ASSERT_TRUE(disk_cache->IsCached(key_100));
  ASSERT_TRUE(disk_cache->IsCached(key_200));
...
TEST_F(DiskCacheManagerTest, CleanupExpire) {
  ...
  auto block = BlockBuilder().Build(std::string(10, '0'));
  auto ctx = BlockContext(BlockFrom::CTO_FLUSH);
  ASSERT_EQ(disk_cache->Stage(key, block, ctx), Errno::OK);
