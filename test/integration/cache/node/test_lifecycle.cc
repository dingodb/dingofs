...
TEST_F(NodeTest, StartsAndStaysAlive) {
  CacheNode node;
  ASSERT_TRUE(node.Start(workdir_ + "/node1", mds_.Addr(), kGroup,
                         utils::GenerateUUID())
                  .ok());
...
TEST_F(NodeTest, PingRespondsAfterStartup) {
  CacheNode node;
  ASSERT_TRUE(node.Start(workdir_ + "/nodep", mds_.Addr(), kGroup,
                         utils::GenerateUUID())
                  .ok());
...
TEST_F(NodeTest, RestartsCleanly) {
  {
    CacheNode node;
    ASSERT_TRUE(node.Start(workdir_ + "/n_a", mds_.Addr(), kGroup,
                           utils::GenerateUUID())
                    .ok());
