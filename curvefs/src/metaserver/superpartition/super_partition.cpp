...
MetaStatusCode SuperPartition::SetFsQuota(uint32_t fs_id, const Quota& quota) {
  MetaStatusCode rc;
  LogGuard log([&]() {
    return StrFormat("set_fs_quota(%d,%s): %s", fs_id, quota.ShortDebugString(),
                     StrErr(rc));
  });
...
MetaStatusCode SuperPartition::FlushFsUsage(uint32_t fs_id,
                                            const Usage& usage) {
  MetaStatusCode rc;
  LogGuard log([&]() {
    return StrFormat("flush_fs_usage(%d,%s): %s", fs_id,
                     usage.ShortDebugString(), StrErr(rc));
  });

  rc = store_->FlushFsUsage(fs_id, usage);
  return rc;
}
...
MetaStatusCode SuperPartition::SetDirQuota(uint32_t fs_id,
  ...
  LogGuard log([&]() {
    return StrFormat("set_dir_quota(%d,%d,%s): %s", fs_id, dir_inode_id,
                     quota.ShortDebugString(), StrErr(rc));
  });
...
MetaStatusCode SuperPartition::GetDirQuota(uint32_t fs_id,
  ...
  LogGuard log([&]() {
    return StrFormat("get_dir_quota(%d,%d): %s (%s)", fs_id, dir_inode_id,
                     StrErr(rc), quota->ShortDebugString());
  });
...
//     optional uint64 usedBytes = 3;
//     optional uint64 usedInodes = 4;
// };
//
// message Usage {
...
MetaStatusCode SuperPartition::CheckQuota(Quota quota) {
  if (!quota.has_usedbytes() && !quota.has_usedinodes()) {
    return MetaStatusCode::PARAM_ERROR;
  } else if (quota.has_usedbytes() || quota.has_usedinodes()) {
    return MetaStatusCode::PARAM_ERROR;
  }
