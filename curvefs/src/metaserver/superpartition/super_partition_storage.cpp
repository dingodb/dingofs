...
MetaStatusCode SuperPartitionStorageImpl::SetFsQuota(uint32_t fs_id,
  ...
  if (quota.has_maxbytes()) {
    old.set_maxbytes(quota.maxbytes());
  }
  if (quota.has_maxinodes()) {
    old.set_maxinodes(quota.maxinodes());
  }
...
MetaStatusCode SuperPartitionStorageImpl::SetDirQuota(uint32_t fs_id,
                                                      ...
                                                      const Quota& quota) {
  return DoSetDirQuota(fs_id, dir_inode_id, quota);
}
...
MetaStatusCode SuperPartitionStorageImpl::LoadDirQuotas(uint32_t fs_id,
  ...
  auto iterator = kv_->SSeek(table_name_, prefix.SerializeToString());
  ...
  for (iterator->SeekToFirst(); iterator->Valid(); iterator->Next()) {
    ...
    std::string svalue = iterator->Key();
...
MetaStatusCode SuperPartitionStorageImpl::DoSetFsQuota(uint32_t fs_id,
                                                       const Quota& quota) {
  return QuotaError(kv_->SSet(table_name_, GetFsQuotaKey(fs_id), quota));
}

MetaStatusCode SuperPartitionStorageImpl::DoGetFsQuota(uint32_t fs_id,
                                                       Quota* quota) {
  return QuotaError(kv_->SGet(table_name_, GetFsQuotaKey(fs_id), quota));
}
...
MetaStatusCode SuperPartitionStorageImpl::DoSetDirQuota(uint32_t fs_id,
                                                        ...
                                                        const Quota& quota) {
  return QuotaError(
      kv_->SSet(table_name_, GetDirQuotaKey(fs_id, dir_inode_id), quota));
}
...
MetaStatusCode SuperPartitionStorageImpl::DoGetDirQuota(uint32_t fs_id,
                                                        ...
                                                        Quota* quota) {
  return QuotaError(
      kv_->SGet(table_name_, GetDirQuotaKey(fs_id, dir_inode_id), quota));
}

MetaStatusCode SuperPartitionStorageImpl::DoDeleteDirQuota(
    uint32_t fs_id, uint64_t dir_inode_id) {
  return QuotaError(
      kv_->SDel(table_name_, GetDirQuotaKey(fs_id, dir_inode_id)));
}
...
MetaStatusCode SuperPartitionStorageImpl::DoFlushDirUsage(Transaction txn,
  ...
  Quota old;
  auto rc = QuotaError(
      txn->SGet(table_name_, GetDirQuotaKey(fs_id, dir_inode_id), &old));
  if (rc != MetaStatusCode::OK) {
    return rc;
  }
  ...
  old.set_usedinodes(used_inodes);
  return QuotaError(
      txn->SSet(table_name_, GetDirQuotaKey(fs_id, dir_inode_id), old));
}
